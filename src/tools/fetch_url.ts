...
function stripHtml(html: string): string {
	return (
		html
			...
			.replace(
				/<\/(p|div|h[1-6]|li|tr|br|hr|section|article|header|footer|pre|code|table)\s*>/gi,
				"\n",
			)
			.replace(/<(br|hr)\s*\/?>/gi, "\n")
			...
			try {
				resp = await fetch(check.url, {
					...
					headers: {
						...
						Accept:
							"text/html,text/plain,application/json,text/markdown;q=0.9,*/*;q=0.5",
					},
				});
			} catch (e) {
				...
				if (
					...
					return err(
						`Fetch timed out after ${FETCH_TIMEOUT_MS}ms: ${params.url}`,
					);
				}
