 ...
 * không fallback SHA (fix BUG 5). Title/description không có trong CI env → rỗng,
 * performReview enrich qua fetchMr().
 *
...
export function mrContextFromEnv(
	...
	return {
		...
		// CI_MERGE_REQUEST_SOURCE_BRANCH_SHA chính xác — không fallback (khác BUG 5).
		sourceSha: env.CI_MERGE_REQUEST_SOURCE_BRANCH_SHA,
		targetSha: env.CI_MERGE_REQUEST_TARGET_BRANCH_SHA,
