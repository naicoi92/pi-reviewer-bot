...
const MAX_TOOL_CALLS = Number(process.env.MAX_TOOL_CALLS_PER_REVIEW ?? 30);
...
function withCallCount(tool: AnyTool, ctx: ToolContext): AnyTool {
  ...
  return {
    ...
    async execute(...args: Parameters<typeof original>) {
      ...
      if (ctx.state.toolCallCount > MAX_TOOL_CALLS) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Tool call cap reached (${MAX_TOOL_CALLS}). Stop calling tools and post your summary now.`,
            },
          ],
          details: {} as never,
        };
