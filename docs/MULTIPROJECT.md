...
### CI wait mode và semaphore

Khi project bật `ci.require: true`, review đang **đợi CI** lưu trong Map riêng (`src/ciwait.ts`), **KHÔNG giữ slot semaphore**. Nghĩa là:

- 3 MR chờ CI không ảnh hưởng 3 slot review active
- Semaphore chỉ được acquire khi review thực sự chạy (sau khi CI pass)
- `/stats` có field riêng `ciWait.pending` cho số review đang đợi

→ Có thể set `ci.require: true` cho nhiều project mà không lo slot chờ CI "ăn" slot review.
...
| Nhiều MR đang đợi CI (ci.require=true) | Không cần tăng MAX_CONCURRENT (CI wait không giữ slot) — chỉ tăng khi review active bị queue |
...
CI_WAIT_TIMEOUT_MS=600000        # 10 phút timeout đợi CI (project có thể override per-project)
...
- `[review !N] ci: pipeline running — enqueued, will review on pipeline success (timeout Xms)` ← chỉ khi ci.require=true
...
- `[webhook] pipeline success <projectId>@<sha8> — triggering deferred review for !N`

→ Đủ để debug bất kỳ issue nào từ log, kể cả CI wait flow.
