...
| D21 | Defense-in-depth SHA resolve qua `fetchMr().diff_refs` | BUG: trong merged-result / edge-case pipeline, `CI_MERGE_REQUEST_DIFF_BASE_SHA` (D18 fallback) cũng empty → `ctx.targetSha` undefined → `postDiffNote` (`gitlab.ts:279`) block **toàn bộ** inline comments với cùng error → AI stall + retry vô ích (4× fail trong log). Fix: `performReview` đã gọi `fetchMr` để enrich title/description — tận dụng luôn để enrich SHA từ `diff_refs.{base_sha, head_sha}` (authoritative source, GitLab UI dùng chính fields này cho DiffNote position). Layer order: CI env ưu tiên (KHÔNG override) → D18 fallback → `diff_refs` fallback (D21). Thêm `enrichShaFromMr(ctx, mr)` pure helper trong `context.ts` + `diff_refs` field trong `MergeRequestObjectAttributes` type. |
